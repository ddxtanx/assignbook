...
function passCompare(pass, user, req, res){
    ...
    bcrypt.compare(pass, userPassHash, (err, same) => {
...
function createResetRequest(user, email){
    ...
    const siteId = random(10).join("");
    const NewReset = new Reset({
        "siteId": siteId,
        "userId": userId
    });

    NewReset.save((err, resp) => {
        if(err) {
            throw err;
        }
...
function findUserForReset(userId, req, res){
    User.findOne({
        ...
        changePass(userId, pass, user.email, () => {
            Reset.remove({
                "userId": userId
            });
...
function comparePasswordsForChange(oldPass, newPass, userId, email, user, res){
    bcrypt.compare(oldPass, user.password, (err, same) => {
        if(err) {
            throw err;
        }
        if(same){
            changePass(userId, newPass, email, () => {
